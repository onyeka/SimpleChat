...
class ChatClient(object):
    ...
    def peer_communication(self, peer_name, peer_msg):
        ...
        if self.DBG: print "=========== got all parts requesting client connection"
        if self.DBG: print "peer ticked:", peer_ticket
        ...
        if self.DBG: print "recieved peer contribution response:", response
        response = response.split(":")
        ...
        if response[0] == "PEER_CHALLENGE":
            ...
            if response[0] == "PEER_ACKNOWLEDGE":
                ...
                if self.DBG: print "=======recieved acknowledgement sending message across, MSG: ", peer_msg
                ...
                if response[0] == "PEER_CONNECT_REQUEST":
                    ...
                    if response[0] == "PEER_CONTRIBUTION":
                        ...
                        if not response[0] == "PEER_CHALLENGE_RESPONSE" and int(response[1]) == challenge + 1:
                            return ret
                        if self.DBG: print "response is correct sending acknowledgement"
                            ...
                            if event == chatClient.getSocket():
                                ...
                                if len(msgSplit) == 1:
                                    ...
                                    if msgSplit[0] == 'send':
                                        ...
                                        new_msg = ''
                                        for word in msgSplit[2:]:
                                            new_msg = new_msg + " " + word
                                        chatClient.peer_communication(msgSplit[1], new_msg)
                                else:
                                    print "Invalid message: %s\n %s" % (msg ,chatClient.CHAT_USAGE)
